#!/usr/bin/env python3
"""
Script to generate authentic Nigerian LGA data from Wikipedia source
This script creates real LGA names for all Nigerian states
"""
...
# Authentic Nigerian LGAs from Wikipedia
AUTHENTIC_LGAS = {
    "Abia": [
    ...
    "Adamawa": [
        "Demsa", "Fufure", "Ganye", "Guyuk", "Girei", "Gombi", "Hong", 
...
def generate_authentic_lgas_sql():
    """Generate SQL for authentic LGAs from Wikipedia data"""
    sql_lines = ["-- Authentic Nigerian LGAs Data (from Wikipedia source)"]
    sql_lines.append("INSERT INTO lgas (id, state_id, name, code) VALUES")
    
    lga_entries = []
    lga_counter = 1
    
    for state_name, lgas in AUTHENTIC_LGAS.items():
        for i, lga_name in enumerate(lgas):
            lga_id = generate_uuid()
            lga_code = f"LGA-{lga_counter:04d}"
            # Escape single quotes in LGA names
            escaped_lga_name = lga_name.replace("'", "''")
            lga_entries.append(f"    ('{lga_id}', (SELECT id FROM states WHERE name = '{state_name}'), '{escaped_lga_name}', '{lga_code}')")
            lga_counter += 1
...
def main():
    print("Generating authentic Nigerian LGA data from Wikipedia source...")
    
    # Count total LGAs
    total_lgas = sum(len(lgas) for lgas in AUTHENTIC_LGAS.values())
    print(f"Total authentic LGAs found: {total_lgas}")
    
    # Generate LGAs SQL
    lgas_sql = generate_authentic_lgas_sql()
    with open('seed_authentic_lgas.sql', 'w') as f:
        f.write(lgas_sql)
    
    print(f"✅ Generated authentic LGA data!")
    print(f"📁 File created: seed_authentic_lgas.sql")
    print(f"📊 Total LGAs: {total_lgas}")
    
    # Verify counts per state
    print("\n📈 LGA counts per state:")
    for state, lgas in AUTHENTIC_LGAS.items():
        print(f"   {state}: {len(lgas)} LGAs")
